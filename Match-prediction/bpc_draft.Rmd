...
atp_results%>%
 select(Player1,Player2,Surface) %>%  
  filter(Player1%in%atp_rankings$Player & Player2%in%atp_rankings$Player)->atp_top_100
wta_results%>%
   select(Player1,Player2,Surface)%>%
filter(Player1%in%wta_rankings$PLAYER & Player2%in%wta_rankings$PLAYER)->wta_top_100
```
...
atp_top_100%>%
	dplyr::mutate(
		player0 = ifelse(Player1 <= Player2, Player1, Player2), # Make player0 first alphabetically
		player1 = ifelse(Player1 <= Player2, Player2, Player1)
	) %>%	group_by(player0, player1) %>%
	dplyr::summarise(	n = n(),y = sum(player0 == Player1),.groups = "drop") %>%
...
wta_top_100%>%
	dplyr::mutate(
		player0 = ifelse(Player1 <= Player2, Player1, Player2), 
		player1 = ifelse(Player1 <= Player2, Player2, Player1)
	) %>%	group_by(player0, player1) %>%
	dplyr::summarise(	n = n(),y = sum(player0 == Player1),.groups = "drop") %>%
  rename(total_count=n,
