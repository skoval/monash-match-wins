...
DT::datatable(options =list(pageLength=10,autoWidth=FALSE),filter = 'top',caption = "The most highest estimated ability of ATP players in different surface" )%>%
  formatRound(columns=c('Mean'), digits=4)
```
...
wta_hard%>%head(10)%>%rbind(wta_clay%>%head(10))%>%
 rbind(wta_grass%>%head(10))%>%
  ...
  mutate(Surface=as.factor(Surface))%>%
DT::datatable(options =list(pageLength=10,autoWidth=FALSE),filter = 'top',caption = "The most highest estimated ability of WTA players in different surface" )%>%
  formatRound(columns=c('Mean'), digits=4)
```
...
```{r atp-sf-cali,fig.cap="Calibration table for evaluating the random effect in WTA data"}
...
atp_sf_cali%>%
  ...
  datatable(options = list(pageLength = 10),filter = 'top', caption = 'Bradley Terry model with random effect in ATP data')%>%
