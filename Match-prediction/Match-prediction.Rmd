...
In the next section, a variety of prediction methods will be introduced formally, and evaluated the estimates for parameters of the model and model performance.
...
Therefore, using the`BTm` function from the package to construct the Bradley Terry model with ATP and WTA ranking data and the `BTabilities` function will computes the baseline ability of each player from the model objects such as Adrian Mannarino and Ajla Tomljanovic became the baseline against with other players in the ATP and WTA data.
