...
wta_abilities$Player <- factor(wta_abilities$Player, 
                        levels = atp_abilities$Player[order(wta_abilities$Ability)],
                        order = T)
wta_match%>%
...
atp_top_100%>%
  ...
  mutate(y=as.numeric(0))->atp_surface
wta_results%>%select(player0,player1,Surface)%>%
  mutate(y=as.numeric(0))->wta_surface
