---
title: "WHO WILL WIN?: Predicting match results in professional tennis"
subtitle: Supervised by Stephanie Kovalchik from Zelus Analytics
author: XITONG HE 
...
knitr::opts_chunk$set(echo = FALSE, cache=FALSE, message=FALSE, warning=FALSE,fig.pos = 'H', fig.align ='center',fig.width = 7,fig.height = 5)
...
data <- data.frame(
  ...
  Description = c("Match Surface","Match Winner","Match Loser","Professional tennis tour tournament" ))
...
On the other hand, small sample size is typical of head-to-head effects in professional tennis. From the figure \@ref(fig:h2h-match) shows the players who have met each other more than 10 times in the past decade. Only a few high ranked players met each other frequently and the rest only meet a few times. In the ATP data, Djokovic met Nadal 29 times, as the current number one and third male player in the world rankings. It is clear that it has fewer players who have met each other more than 10 times in the WTA data. The players who faced each other most often are Serena and Azarenka,which have met 15 times.

```{r h2h-match,fig.cap="The players who have met each other more than 10 times"}

h2h_count(atp_results,player0,player1)%>%
  rename(total_count=n,
         p0_count=y)%>%
  arrange(-total_count)%>%
  filter(total_count>=10)%>%
  mutate(Player=paste0(Player0,":",Player1))%>%
  select(Player,total_count)->atp_full_h2h
 
h2h_count(wta_results,player0,player1)%>%
  rename(total_count=n,
         p0_count=y)%>%
  arrange(-total_count)%>%
  filter(total_count>=10)%>%
  mutate(Player=paste0(Player0,":",Player1))%>%
  select(Player,total_count)->wta_full_h2h

freq1<-atp_full_h2h%>%
  mutate(angle = 90 * sample(c(0, 1), n(), replace = TRUE, prob = c(60, 40)))%>%
ggplot(aes(label = Player, size = total_count,angle=angle,
            col=factor(sample.int(10, nrow(atp_full_h2h), replace = TRUE)))) + 
...
freq2<-wta_full_h2h%>%
  mutate(angle = 90 * sample(c(0, 1), n(), replace = TRUE, prob = c(60, 40)))%>%
ggplot(aes(label = Player, size = total_count,angle=angle,
            col=factor(sample.int(10, nrow(wta_full_h2h), replace = TRUE)))) + 
  geom_text_wordcloud(rm_outside = TRUE, max_steps = 1,
                      grid_size = 1, eccentricity = .9)+
  scale_size_area(max_size = 3)+
  scale_color_brewer(palette = "Paired", direction = -1)+
...
ggarrange(freq1,freq2,ncol = 2)
```


```{r tier-surface-function}
...
tier_count(atp_results,player0,player1,Tier)%>%
  ...
  rename(`met 1 time`=match.x,
         `met 2 times`=match.y,
         `met 3+ times`=match)%>%
  pivot_longer(`met 1 time`:`met 3+ times`,names_to="Players meet",values_to="value")->atp_tier

atp_tier%>%
  ggplot(aes(x=`Players meet`,y=value,fill=Tier))+
  geom_col(position="dodge2")+
  theme_bw(base_size = 12) + 
  ggtitle("The number of matches for players met in each tier in ATP")+
  geom_text(aes(label=value), vjust=0, hjust=0.5, position = position_dodge(width = 1),size=3)+
  xlab("Number of times players meet")+
  ylab("Count")

```

```{r wta-tier-plot,fig.cap="The number of matches for players met in different number of times in WTA"}
...
tier_count(wta_results,player0,player1,Tier)%>%
  ...
  left_join(tier_count(wta_results,player0,player1,Tier)%>%
   ...
   rename(`met 1 time`=match.x,
         `met 2 times`=match.y,
         `met 3+ times`=match)%>%
  pivot_longer(`met 1 time`:`met 3+ times`,names_to="Players meet",values_to="value")->wta_tier

wta_tier%>%
  ggplot(aes(x=`Players meet`,y=value,fill=Tier))+
  geom_col(position="dodge2")+
  theme_bw(base_size = 12) + 
  ggtitle("The number of matches for players met in each tier in WTA")+
  geom_text(aes(label=value), vjust=0, hjust=0.5, position = position_dodge(width = 1),size=3)+
  xlab("Number of times players meet")+
  ylab("Count")
...
surface_count(atp_results,player0,player1,Surface)%>%
  ...
  rename(`met 1 time`=match.x,
         `met 2 times`=match.y,
         `met 3+ times`=match)%>%
  pivot_longer(`met 1 time`:`met 3+ times`,names_to="Players meet",values_to="value")->atp_surface


atp_surface%>%
  ggplot(aes(x=`Players meet`,y=value,fill=Surface))+
  geom_col(position="dodge2")+
    theme_bw(base_size = 12) + 
  ggtitle("The number of matches for players met in each surface in ATP")+
  geom_text(aes(label=value), vjust=0, hjust=0.5, position = position_dodge(width = 1),size=3)+
  xlab("Number of times players meet")+
  ylab("Count")
```

```{r wta-surface,fig.cap="The number of matches for players met in different number of times in each surface in WTA"}

#calculate the percentage of h2h matches in each surface for atp results
surface_count(wta_results,player0,player1,Surface)%>%
  filter(n==1)%>%group_by(Surface)%>%summarise(`met 1 time`=sum(n))%>%
  left_join(surface_count(wta_results,player0,player1,Surface)%>%
              filter(n==2)%>%group_by(Surface)%>%summarise(`met 2 times`=sum(n,na.rm=TRUE)),by=c('Surface'='Surface'))%>%
   left_join(surface_count(wta_results,player0,player1,Surface)%>%
              filter(n>=3)%>%group_by(Surface)%>%summarise(`met 3+ times`=sum(n,na.rm=TRUE)),by=c('Surface'='Surface'))->wta_surface
wta_surface[is.na(wta_surface)] = 0


wta_surface%>%
  pivot_longer(`met 1 time`:`met 3+ times`,names_to="Players meet",values_to="value")%>%
  ggplot(aes(x=`Players meet`,y=value,fill=Surface))+
  geom_col(position="dodge2")+
    theme_bw(base_size = 12) + 
  ggtitle("The number of matches for players met in each surface in WTA")+
  geom_text(aes(label=value), vjust=0, hjust=0.5, position = position_dodge(width = 1),size=3)+
  xlab("Number of times players meet")+
  ylab("Count")
...
$$P_{ij}=P(y_{ij}=1|\delta_{ij})=\frac{exp(\delta_{ij})}{1+exp(\delta_{ij})};$$
$$\delta_{ij}=\lambda_{i}-\lambda_{j}$$
where$\lambda_{i}$ represents the skill level of player i and $\lambda_{j}$ represents the skill level of player j. The conditional probability of ${y_{ij} =0}$ is instead equal to $1-P_{ij}$.
...
atp_abilities%>%
  datatable(options = list(pageLength = 10),filter = 'top',class = "table-hover table-striped cell-border stripe",caption = 'The estimated ability for ATP data')%>%
...
wta_abilities$Player <- factor(wta_abilities$Player, 
                        ...
                        order = T)
wta_abilities%>%
   datatable(options = list(pageLength = 10),filter = 'top',class = "table-hover table-striped cell-border stripe", caption = 'The estimated ability for WTA data')%>%
   formatRound(columns=c('Ability', 'SE'), digits=4)
...
From the table \@ref(fig:ATP-ability) and \@ref(fig:WTA-ability), Adrian Mannarino and Ajla Tomljanovic became the baseline against with other players on the ATP and WTA data as their ability are 0. In the ATP data, Novak Djokovic, Rafael Nadal and Roger Federer have the highest estimated ability compared with the Adrian Mannarino. All three of them have played and won most of matches and their ranking almost maintain at the top 10 of the world on average, indicating that their tennis level are higher than other players. On the contrary, Lorenzo Musetti and Sebastian Korda have the lowest estimated ability and one  the reason for that they have only played one game and even lost in that match, so their ability is estimated quite lower. 
...
#set up the function for constructing the stan model data
...
- Rhat: the potential scale reduction statistic
  - `Rhat` statistic is roughly the ratio of the variance of a parameter when the data is pooled across all of the chains to the within-chain variance so it can helps tell us whether these parameters are so poorly sampled and measures the extent to which chains are reaching different conclusions. @vehtari2021rank stated the Rhat should be in the range of 0.9 to 1.05 as the more idiosyncratically the chains behave if the Rhat value is far away from 1.  Figure \@ref(fig:check-rhat) displays the distribution of Rhat value in two Stan models and most of Rhat value are close to 1, implying that all chains have been converged to the same systematic behavior for a given parameter. So, the R hats look quite promising overall and suggest that the target posterior has been reached for the model parameters.
...
wta_h2h%>%
  ...
  select(player0_rank,player1_rank,player0,player1,mean)%>%
  arrange(desc(mean))%>%
  datatable(options = list(pageLength = 10),filter = 'top',class = "table-hover table-striped cell-border stripe",caption = 'The head-to-head effect in different pair of players in WTA')%>%
   formatRound(columns=c('mean'), digits=4)
```
...
wta_lookup%>%
  ...
  separate(Parameter,c("U1","index","cluster"))%>%
    mutate(index=as.numeric(index))%>%
  filter(cluster=="4"),by=c("Index"="index"))%>%
  arrange(desc(Mean))%>%
  mutate(Surface=paste("Carpet"))%>%
  select(Names,Ranking,Mean,Surface)->wta_carpet
```
...
In the Bradley-Terry model, using the `predict` function to predict the probability of player 0 win in the competition between each pair of players, and then grouping the expected probability into 10 bins to calculate the mean of expected probability in each bin. Table \@ref(tab:cali-bt-table) shows the comparison between the mean actual probability and the mean  predictive probability in each bin in the ATP and WTA. The larger the positive difference is, the greater the underestimation of the winning rate is. At the same time, the larger the negative difference is, the more overestimated the winning rate is. In order to directly visualize the difference between the expected value against with actuality, the scatter plot \@ref(fig:cali-bt-plot) is constructed as the calibration curve to evaluate the model performance.

Therefore, from the figure \@ref(fig:cali-bt-plot), the estimated winning probability of player 0 is close to the actual value, especially in the range of 0.5 to 0.75 in ATP data. There is a little bit of volatility outside the range. Looking back at the table \@ref(tab:cali-bt-table) , it is obvious that the difference between the expected value against with reality is smaller and positive except in the bins 2,3,5 and 9. The biggest differences is in the bins 2 and it can be visualized in the plot as well where has the most fluctuating in the 12%-15%.
...
Although there are differences in each box, the difference is small and can be ignored. Moreover, most of the predicted values in the scatter plot \@ref(fig:cali-bt-plot) fit well with the actual probability. Overall, the model prediction is close to the actual probability of player 0 wins. 
...
bin_freq<-function(data){
  data%>%
  group_by(bins)%>%
  summarise(mean_pred=sum(y_pred)/n())
}
```

```{r cali-bt-table,fig.cap="Calibration table with standard Bradley-Terry Model"}
atp_match%>%
...
atp_bt_cali%>%
  ...
  kable(caption = "Calibration table for standard Bradley-Terry model",digits = 4) %>%
  kable_styling(bootstrap_options = 
                  c("striped", "condensed"), 
                  full_width = T, 
                  position = "center")
...
It is clear that there are two segments of the WTA data with the greatest volatility in the figure \@ref(fig:cali-stan-plot), one trend to be concave, the other one is convex. The fluctuation in ATP data seems to be lesser than WTA data and only the first and last bins deviated from the 45 degree line, and the rest seemed to fit well.  

Rather than only visualizing the relationship between expected probability and actual probability from the figure, combine with the table \@ref(tab:cali-stan-table) to analyze why such a situation appears. Based on the table \@ref(tab:cali-stan-table), the most interesting thing for WTA data is in the last two bins where the mean actual probability is 1 in the ninth bin and the mean predicted probability is 1 in the tenth bin, causing the convex issue. In fact, the plot \@ref(fig:wta-cali-freq) which can be interactively visualized the distribution of number of matches with different times of meeting in each bin in WTA data and it is clear that in the ninth bin, only one pair player like Victoria Azarenka with Alize Cornet appears and Azarenka won all matches in 6 times but the predicted matches of Azarenka win are 5, which causes the difference between mean of actual probability and expected value. However, the frequency of most of the matches in the tenth bin is concentrated in 1 and there are only 54 matches that players have met twice, suggesting that the matchup effects among them who only have played one time are unstable. A similar situation for ATP data can be seen in the plot \@ref(fig:atp-cali-freq), there are more than 400 matches that players have played one time and only 27 matches that players have played more than three time, indicating that the size of players met one time is larger than players met more than three times so that the matchup effects between them are abnormal or variable. 

Another point of concern is that the mean predicted probability are zero in both first bins. One of the reasons is that the sample size of the matchup effects is extremely small. It can be seen from the figure \@ref(fig:atp-cali-freq) and \@ref(fig:wta-cali-freq), there are more than 400 matches that players have only met one time with their opponent in the ATP and WTA, indicating that head-to-head effects between them are less than other which causes all the predicted matches of player 1 win are zero in this case.
...
```{r atp-cali-freq,fig.cap="The distribution of the number of matches that ATP players meet in the number of times"}
library(plotly)
atp_freq<-atp_stan_bins%>%
  group_by(bins,total_count)%>%
  tally()%>%
mutate(matches=as.character(case_when(total_count==1 ~"Met 1 time",
                             total_count==2 ~"Met 2 times",
                             total_count>=3~"Met more then 3 times")))%>%
  rename(count=n)%>%
  ggplot(aes(bins,count,fill=matches))+
   geom_col(position = "dodge")+
  theme_bw(base_size = 12) +
 xlab("bins")+
  ylab("Count")+
  ggtitle("The distribution of the number of matches for ATP players")+
...
```{r wta-cali-freq,fig.cap="The distribution of the number of matches that WTA players meet in the number of times"}
wta_freq<-wta_stan_bins%>%
  group_by(bins,total_count)%>%
  tally()%>%
mutate(matches=as.character(case_when(total_count==1 ~"Met 1 time",
                             total_count==2 ~"Met 2 times",
  ...
  xlab("bins")+
  ylab("Count")+
  ggtitle("The distribution of the number of matches for WTA players")+
  scale_x_continuous(breaks = seq(1, 10,1))
ggplotly(wta_freq)
...
Since only a few matches happened in the carpet court which may raise more uncertainty for evaluating this surface type in the model, it can give more explanation by combining carpet and grass court together. So, the figure \@ref(fig:cali-surface-plot) can be visualized directly how well the model fit in the data which has been considered the surface random effect. Based on the scatter plot, the performance of the mixed effect models in the data is relatively poor especially in WTA data. The volatility between prediction and actual probability in WTA is larger than in ATP and all the predicted values are inconsistent with the reality or even close to the actual value in both plots. 

Further explanation needs to be explored from the table \@ref(fig:atp-sf-cali) and \@ref(fig:wta-sf-cali) which display the mean of expected probability with different types of surface against the actual probability in each bin in ATP data and WTA data. In the table \@ref(fig:atp-sf-cali), the mean of expected probability increase gradually in all surfaces, but the actual value seems to be concentrated in 0.4 to 0.5 in the third to eighth bin. One of the interesting things is the difference between expected probability and actuality are smallest in the fifth bin in three surfaces. Before the fifth bin, the mixed effect model underestimated the probabilities of player 0 wins, but after that, the probabilities of player 0 wins are overestimated.

On the other hand, in the table \@ref(fig:wta-sf-cali),the differences between expected probability and real value in WTA are completely different from the ATP table as the probabilities of player 0 wins in all surfaces are overestimated from the mixed effect model before the fifth bin and are underestimated after the fifth bin. It is worth noting that the expected probability of player 0 wins in the third bin is the most underestimated, with an average probability of about 65%, while the average predicted value is only 26%. That is the peak in the grass court that is shown in the right side of scatter plot \@ref(fig:cali-surface-plot).
...
#generate the predicted probability in each matches
predict(atp_random,newdata = atp_top_surface%>%select(-y),type="repsonse")%>%
  as_tibble()->atp_spred

atp_top_surface%>%
  select(-y)%>%
  cbind(colMeans(atp_spred%>%
           select(starts_with("y_pred")))%>%
  as_tibble())->atp_spred

#calculate the mean of predicted probability for each pair of players
atp_spred%>%
  group_by(player0,player1,Surface)%>%
  summarise(pred_mean=sum(value)/n(),p0_count=n())->new_atp_spred

new_atp_spred%>%
  right_join(atp_sf,by=c("player0"="Player0","player1"="Player1","Surface"="Surface","p0_count"="y"))->atp_sf_pred

atp_sf_pred%>%
  filter(is.na(pred_mean))%>%
  left_join(new_atp_spred,by=c("player0"="player1","player1"="player0","Surface"="Surface"))%>%
  select(player0,player1,Surface,pred_mean.y,p0_count.x,n)%>%
  rename(pred_mean=pred_mean.y,p0_count=p0_count.x)%>%
  rbind(atp_sf_pred%>%drop_na())%>%
  rename(y_pred=pred_mean)->atp_sf_pred


predict(wta_random,newdata = wta_top_surface%>%select(-y),type="repsonse")%>%
  as_tibble()->wta_spred

wta_top_surface%>%
  select(-y)%>%
  cbind(colMeans(wta_spred%>%
           select(starts_with("y_pred")))%>%
  as_tibble())->wta_spred
...
wta_spred%>%
  group_by(player0,player1,Surface)%>%
  summarise(pred_mean=sum(value)/n(),p0_count=n())->new_wta_spred



new_wta_spred%>%
  right_join(wta_random,by=c("player0"="Player0","player1"="Player1","Surface"="Surface","p0_count"="y"))->wta_sf_pred

wta_sf_pred%>%
  filter(is.na(pred_mean))%>%
  left_join(new_wta_spred,by=c("player0"="player1","player1"="player0","Surface"="Surface"))%>%
  select(player0,player1,Surface,pred_mean.y,p0_count.x,n)%>%
  rename(pred_mean=pred_mean.y,p0_count=p0_count.x)%>%
  rbind(wta_sf_pred%>%drop_na())%>%
  rename(y_pred=pred_mean)->wta_sf_pred

#write_csv(atp_sf_pred,"atp_surface_pred.csv")
#write_csv(wta_sf_pred,"wta_surface_pred.csv")
```



```{r read-surface-prediction}
atp_sf_pred<-read_csv(here("data/atp_surface_pred.csv"))
wta_sf_pred<-read_csv(here("data/wta_surface_pred.csv"))
```


```{r atp-sf-cali,fig.cap="Calibration table for evaluating the random effect in ATP data"}

bins(atp_sf_pred)->atp_sf_bins

atp_sf_bins%>%
  group_by(bins,Surface)%>%
  summarise(mean_pred=sum(y_pred)/n())->atp_sf_freq

atp_sf_bins%>%
  group_by(bins,Surface)%>%
  summarise(mean_act=sum(p0_count)/sum(n))%>%
  left_join(atp_sf_freq)%>%
  mutate(Surface=as.factor(Surface))->atp_sf_cali


atp_sf_cali%>%
  rename(atp_mean_act=mean_act,
         atp_mean_pred=mean_pred)%>%
  mutate(diff=atp_mean_act-atp_mean_pred)%>%
  arrange(desc(Surface))%>%
  datatable(options = list(pageLength = 10),filter = 'top' ,class = "table-hover table-striped cell-border stripe",caption = 'Bradley Terry model with random effect in ATP data')%>%
  formatRound(columns=c('atp_mean_act', 'atp_mean_pred','diff'), digits=4)

```

```{r wta-sf-cali,fig.cap="Calibration table for evaluating the random effect in WTA data"}
bins(wta_sf_pred)->wta_sf_bins

wta_sf_bins%>%
  group_by(bins,Surface)%>%
  summarise(mean_pred=sum(y_pred)/n())->wta_sf_freq

wta_sf_bins%>%
  mutate(Surface=as.character(case_when(Surface=="Hard"~"Hard",
                                        Surface=="Clay"~"Clay",
                                        Surface=="Grass"~"Grass",
                                        Surface=="Carpet"~"Grass")))%>%
  group_by(bins,Surface)%>%
  summarise(mean_act=sum(p0_count)/sum(n))%>%
  left_join(wta_sf_freq)%>%
  mutate(Surface=as.factor(Surface))->wta_sf_cali

wta_sf_cali%>%
  rename(wta_mean_act=mean_act,
         wta_mean_pred=mean_pred)%>%
  mutate(diff=wta_mean_act-wta_mean_pred)%>%
  arrange(desc(Surface))%>%
  datatable(options = list(pageLength = 10),filter = 'top',class = "table-hover table-striped cell-border stripe",caption ='Bradley Terry model with random effect in WTA data')%>%
  formatRound(columns=c('wta_mean_act', 'wta_mean_pred','diff'), digits=4)
```


```{r cali-surface-plot,fig.cap="Calibration curve for surface random effect in bayesian paired comparison model" }

p5<-ggplot(atp_sf_cali,aes(mean_pred,mean_act,color=Surface))+
  geom_line()+
  geom_abline(slope = 1, intercept = 0, linetype = 2) +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 1)) +
  theme_bw()+
...
p6<-ggplot(wta_sf_cali,aes(mean_pred,mean_act,color=Surface))+
  geom_line()+
  geom_abline(slope = 1, intercept = 0, linetype = 2) +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 1)) +
  theme_bw()+
...
This project seeks to develop and compare the different approaches for predicting the tennis match outcomes. The main method to construct the model structure is the Bradley-Terry model which is one of the most effective ways to apply the paired comparison model [@mchale2011bradley]. Based on this, the head-to-head effect would be considered as the explanatory factor in the model so that the Bayesian paired comparison model was introduced by capturing these matchup effects. In addition, not only considering the head-to-head effect, but also exploring the ability of each player in different types of surface. The mixed effect model with surface random effects was implemented in this project for achieving the goal. After modeling the data, the calibration would be evaluated the model fitting and model performance in three models.

For the standard Bradley-Terry model, the model fitting was good and it trained the data to evaluate the ability of each player. The model performance in the ATP and WTA data was well and the predicted probabilities were closer to the actual value. However, the standard Bradley-Terry model only implemented based on the match level information such as the number of matches with player 0 wins against with player 0 loses, which was not enough to construct the prediction model for tennis matches[@jackson1997heavy]. The result of the player - opponent match sometimes is not only about the different capability between two players, it also about the playing style or abilities in different surface courts.
...
The other improvement of this project is to take player ranking and Elo score as exploratory factors in the model, which can improve the accuracy and practicability of tennis prediction. Additionally,the Elo-based prediction system created by the website FiveThirtyEight.com[@morris2016we], which would be considered as the further model development well[@vaughan2021well].


# Acknowlegments

The R packages are used in this report: 
tidyverse[@tidyverse]; dplyr[@dplyr]; ggplot2[@ggplot2]; here[@here]; kableExtra[@kableExtra]; ggwordcloud[@ggwordcloud]; scales[@scales]; formattable[@formattable]; bpcs[@bpcs]; BradleyTerry2[@BradleyTerry2]; rvest[@rvest]; rstan[@rstan]; bayesplot[@bayesplot]; ggpubr[@ggpur]; DT[@DT];plotly[@plotly]
