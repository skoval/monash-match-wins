...
Pairwise comparison is the process of comparing entities in pairs to determine which one has a better chance of winning. One of the famous and more effective method for establishing pairwise comparison is Bradley Terry Model[@bradley1952rank], applying in the contest between two players.
...
where $$\lambda_{i}$$=log$$\alpha_{i}$$ for all i and is the respective skill parameter of the players. Optimization can be used to jointly solve for skills parameters of all players based on a fixed period of historical results. Match winning probabilities can then be derived based on the same IID assumption used in point models. Therefore, based on the i.i.d(independent identifying distributed) assumption for all contests[@klaassen2001points],the parameters $$\lambda_{i}$$ can be estimated by maximum likelihood using `BradleyTerry2` package[@bradley] for generalized linear models. 
...
* Observed variables: The tennis matches outcome. A single outcome will be given the notation r and a set of outcomes D.
...
* prior probability: P(w) A prior probability distribution on the parameters of the model P(w).A standard normal prior is used in this project.

For some data D, on a set of n outcomes, the posterior probability of the model parameters can be expressed as:
$$P(w|D)=\frac{P(D|w)P(w)}{P(D)}$$
where P(D|w) is the likelihood of the model given n the data and P(D) is marginal likelihood of the model found through normalisation. Under the assumption that outcomes are independent identified distributed, the likelihood can be expressed as:
...
Applying Bayesian method in Bradley Terry Model, the extension formula is shown as below:

$$\mathrm{P}(\left[ i \mbox{ beats } j \right])=\frac{exp(\lambda_{i})}{exp(\lambda_{i})+exp(\lambda_{j})}=\frac{1}{1+e^{-(\lambda_{i}-\lambda_{j})}}=sigmoid(\lambda_{i}-\lambda_{j}) $$

where$$\lambda_{i}$$ and $$\lambda_{j}$$ are the player skills which are the parameters of the model, and sigmoid is the logistic sigmoid function: $$\frac{1}{1+e^{-x}}$$. 
...
$$P(D|\lambda)=\prod_{i = 1}^{N} (sigmoid(\lambda_{a}^i-\lambda_{b}^i))$$
where $$\lambda_{a}$$ and $$\lambda_{b}$$ are the respective abilities of the winning and losing player of the $$i$$outcomes.
...
#scatter plot
ggplot(atp_cali,aes(mean_act,mean_pred))+
  geom_line()+
...
- `stan model script`: Instead of only using simple Bradley Terry model with bernoulli distribution in a single match,the Stan model data collapse the matches between every pair of players have been became the binomial distribution. 

The parameters are used in the stan model:
  *n: is the N trials 
  *y: is the number of success which is the number of times that player0 has won the match in this case. 
  ...
  * `p1_win`: is given chance of success which is evaluated from the Bradley Terry Model where is simply calculated by `lamdba(player1)-lambda(player1)` but it still need to consider with head-to-head effect between these two players, so the final function for estimating the probability of player1 winning is shown as below:
  
$$\lambda_{player1}-\lambda_{player0}+h2h_{player1,player0}$$
- `vb` function: is used variational inference for approximately posterior distribution. In this case,using VB testing for examining the result whether the model construction and diagnostics is reasonable approximation or not. It can also minimize the difference between actual distribution and approximation distribution and reduce the time consumption, which is a more effective and less uncertain method for checking the Stan model correctly.
...
atp_stan<-atp_match%>%
  rename(n=total_count,y=p0_count)%>%
...
model_data_atp <- make_stan_data(atp_stan, "y", "n", "player0", "player1")
model_data_wta <- make_stan_data(wta_stan, "y", "n", "player0", "player1")
...
# Use VB for testing/ VB for model testing/development
...
#construct the clean cefficient from the output of stan-model
...
atp_fit<-read.csv(here("Match-prediction/atp_sampling.csv"))
wta_fit<-read.csv(here("Match-prediction/wta_sampling.csv"))
...
```{r h2h-atp}
atp_players <- unique(c(atp_stan$player0, atp_stan$player1))
...
atp_stan%>%
  ...
  rename(player0_id=atp_index.x,
         player1_id=atp_index.y)->atp_stan
