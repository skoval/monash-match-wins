...
In the follwoing section, a variety of prediction methods will be introduced formally, and evaluated the estimates for parameters of the model and model performance.
...
Firstly, using the`BTm` function from the package to construct the Bradley Terry model with ATP and WTA ranking data and then the `BTabilities` function will computes the baseline ability of each player from the model objects such as Adrian Mannarino and Ajla Tomljanovic became the baseline against with other players in the ATP and WTA data.
...
```{r p-value}
ll.null<-atp_bt$null.deviance/-2
ll.proposed<-atp_bt$deviance/-2
(ll.null-ll.proposed)/ll.null
1-pchisq(2*(ll.proposed-ll.null),df=(length(atp_bt$coefficients)-1))
...
```{r ceof-BT-model}
atp_bt_pred%>%
  mutate(outcome=as.factor(ifelse(p0_count>p1_count,'win','lose')))%>%
  calibration_curve(outcome, y_pred) %>%
  autoplot()

library(calibcurve)
atp_bt_pred%>%
  mutate(outcome=as.factor(ifelse(p0_count>p1_count,'0','1')))%>%
  calibration_curve(outcome, y_pred) %>%
  autoplot()

library(givitiR)
atp_bt_pred%>%
  mutate(outcome=as.numeric(p0_count<p1_count))->data

cb1 <- givitiCalibrationBelt(o = data$outcome, e = data$y_pred,
                             devel = "external")
...
```{r stan-data-function,eval=FALSE}
...
atp_stan<-atp_top_100%>%
  select(player0,player1)%>%
  mutate(y=as.numeric(0))
wta_stan<-wta_top_100%>%
  select(player0,player1)%>%
  mutate(y=as.numeric(0))

model_data_atp <- make_stan_data(atp_stan, "y", "player0", "player1")
model_data_wta <- make_stan_data(wta_stan, "y", "player0", "player1")
