...
atp_surface%>%
   kable(caption = "Matches with head to head effect on surface in ATP") %>%
...
The main approaches used in this project are fell into two categories:
...
```{r atp-ability,echo=FALSE,fig.width = 8,fig.height = 5,fig.cap="The estimated ability between Nadal and Federer"}
...
<br>
<br>
$P(Win1)= \frac{\exp{(\lambda_1 - \lambda_0)}}{1 + \exp{(\lambda_1 - \lambda_1)}} \to \frac{\exp{(\lambda_1-\lambda_0+h2h_{1,0})}}{1+\exp{(\lambda_{1}-\lambda_{0}+h2h_{1,0})}}$
<br>
<br>
...
The extension of Bradley-Terry model with surface random effects can be represented as:
...
- In general, the next prediction outcome: 
$P(r^{n+1}_{ij}=1|\lambda_{i};\lambda_{j})$ where i beats j given the historical data will be simply inferred as:
$\sigma(\lambda_{i}-\lambda_{j})$
- Implement in Stan with binomial distribution:
<center>
$$y \sim binomial(n,p)$$
$$P_{win_1}=\frac{\exp{(\lambda_1-\lambda_0+h2h_{1,0})}}{1+\exp{(\lambda_{1}-\lambda_{0}+h2h_{1,0})}}$$
